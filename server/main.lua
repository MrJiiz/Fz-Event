...
TriggerEvent('es:addAdminCommand', 'eventdata', 13, function(source, args, user)
    ...
    local xPlayer = FZ.GetPlayerFromId(_source)
	...
	MySQL.Async.fetchAll('SELECT * FROM events_data', {}, function(data)
		if data[1] ~= nil then
			...
			MySQL.Async.execute('DELETE FROM events_data', {}, function(rowsChanged)
				if rowsChanged > 0 then
					TriggerClientEvent('esx:ShowJLNotification', source, 'Shoma Ba Movafaghiyat Event Ra Hazf Kardid')
				end
			end)
		else
			TriggerClientEvent('esx:ShowJLNotification', source, 'Eventi Baraye Pak Kardan Vojod Nadarad!')
		end
	end)
end)
